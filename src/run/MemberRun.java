...
public class MemberRun implements Member {
	...
	public Action login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (member == 0) {
			...
			// 세션에 로그인한 회원 번호 set
			// request.getSession().setAttribute("member_no",
			// MB.getMember_no());
			// request.getSession().setAttribute("member_name",
			// MB.getMember_name());
			request.getSession().setAttribute("loginMemberInfo", MB);
	...
	@Override
	public void adviceCheck(HttpServletRequest request, HttpServletResponse response) throws IOException {

		// 상담신청시 회원 확인
		if (request.getSession().getAttribute("loginMemberInfo") == null) {
			// 로그인 안함
