...
public class FreeBoardRepliedDAO {
	...
	public void add(FreeBoardRepliedBean bean) {
		sql = "select max(FREEBOARD_RE_NO)+1 from FREEBOARD_REPLY";
		int result = 0;
		...
		try {
			...
			try {
				...
				System.out.println("add 롤백 예외 발생");
				e1.printStackTrace();
			}
	...
	public void delete(FreeBoardRepliedBean bean) {
		...
		try {
			...
			System.out.println("부모글 답글수 1감소 예외 발생");
			e.printStackTrace();
			try {
				con.rollback();
			} catch (SQLException e1) {
				System.out.println("delete 롤백 예외 발생");
				e1.printStackTrace();
			}
		} finally {
			sql = "";
