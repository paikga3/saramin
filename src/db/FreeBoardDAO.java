...
public class FreeBoardDAO {
	...
	public void add(FreeBoardWriteBean bean) {
		try {
			sql = "select max(FREEBOARD_NO)+1 from FREEBOARD_WRITE";
	...
	public FreeBoardWriteBean detail(int freeBoard_no) {
		...
		sql = "select FREEBOARD_NO,FREEBOARD_REPLYCOUNT,FREEBOARD_READCOUNT,"
					+ "FREEBOARD_RECOMMAND,FREEBOARD_ALIAS,FREEBOARD_TITLE,FREEBOARD_CONTENTS,"
					+ "DATE_FORMAT(FREEBOARD_REGDATE, '%Y:%m:%d:%H:%i:%s') as FREEBOARD_REGDATE "
	...
	public int getTotalSearchNum(String keyword) {
		Properties prop = new Properties();
		try {
			prop.load(this.getClass().getResourceAsStream("../properties/query.prop"));
			sql = prop.getProperty("searchTotalCount");
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		try {
			...
			System.out.println("검색키워드에 대한 총 글갯수 반환 예외");
			e.printStackTrace();
		} finally {
			sql = "";
			if(rs != null) try { rs.close();} catch (SQLException e) {}
	...
	public ArrayList<FreeBoardWriteBean> getSearchList(String keyword, int serachPage) {
		ArrayList<FreeBoardWriteBean> list = new ArrayList<>();
		Properties prop = new Properties();
		try {
			prop.load(this.getClass().getResourceAsStream("../properties/query.prop"));
			sql = prop.getProperty("search");
		} catch (IOException e1) {
			// TODO Auto-generated catch block
			e1.printStackTrace();
		}
		...
		try {
			...
			int start_rownum = serachPage * 20 - 19;
			int end_rownum = serachPage * 20;
			
			ps.setString(1, "%" + keyword + "%");
			ps.setString(2, "%" + keyword + "%");
			ps.setInt(3, start_rownum);
			ps.setInt(4, end_rownum);
			...
			if(list.size() == 0) {
				System.out.println("검색키워드에 대한 글목록이 없습니다. 빈 컬렉션을 반환합니다.");
			} else {
				System.out.println("검색키워드에 대한 글목록을 반환합니다.");
			}
			return list;
		} catch (SQLException e) {
			System.out.println("검색키워드에 대한 글목록 불러오기 실패");
			e.printStackTrace();
		} finally {
			sql = "";
			if(rs != null) try { rs.close();} catch (SQLException e) {}
